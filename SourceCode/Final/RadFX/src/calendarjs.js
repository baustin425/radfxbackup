 ...
 *
 *
 * Edited extensively for database connection and persistent state
 * Edited by Sean Nuetzmann 03/01/2022
 * Replacement of repeat feature with subevent chain
 * Restriction to add event from request process
 * Cannot add event directly to calendar
 * Cannot remove root event without canceling request
 * Abilility added to approve event or set priority and color event accordingly
 * Functions uploadEvent, uploadDupEvent, removeEvent, and removeDupEvent added
 * Functions async to database with XMLHttp object and force window refresh for hard sync with DOM
 * This is a database first approach to what was a local calendar
 *
 */
 ...
 * @property    {string}    titleText                                   The text that should be displayed for the "Test" label.
...
function calendarJs( id, options, searchOptions, startDateTime ) {
    var _options = {},
		...
		_element_EventEditorDialog_Energy = null,		
		_element_EventEditorDialog_Ion = null,		
		_element_EventEditorDialog_Priority = null,		
    ...
    function getOrderedEvents( events, sortAllDayEvents ) {
		...
		for(var x=0; x < length - 1 ; x++){
			...
			//Default Color Blue
			events[x]['color'] = '#08344e';
			events[x+1]['color'] = '#08344e';
			events[x]['colorText'] = '#FFFFFF';
			events[x+1]['colorText'] = '#FFFFFF';
			
			
			
			//Grey Out Unapproved
			if (events[x]['approved'] == 0 ){
				events[x]['color'] = '';				
			}
			if (events[x+1]['approved'] == 0 ){
				events[x+1]['color'] = '';				
			}
			
			//Bright Green Priority
			if (events[x]['priority'] == 1 ){
				events[x]['color'] = '#E6783D';				
			}else if (events[x]['priority'] == 2 ){
				events[x]['color'] = '#C7430A';				
			}else if (events[x]['priority'] == 3 ){
				events[x]['color'] = '#C2C200';	
				events[x]['colorText'] = '#000000';				
			}else if (events[x]['priority'] == 4 ){
				events[x]['color'] = '#FFF700';	
				events[x]['colorText'] = '#000000';
			}else if (events[x]['priority'] == 5 ){
				events[x]['color'] = '#0C850A';	
				events[x]['colorText'] = '#000000';
			}else if (events[x]['priority'] == 6 ){
				events[x]['color'] = '#00FF00';	
				events[x]['colorText'] = '#000000'				
			}
			
			
			
			if (events[x+1]['priority'] == 1 ){
				events[x+1]['color'] = '#E6783D';				
			}else if (events[x+1]['priority'] == 2 ){
				events[x+1]['color'] = '#C7430A';				
			}else if (events[x+1]['priority'] == 3 ){
				events[x+1]['color'] = '#C2C200';
				events[x+1]['colorText'] = '#000000';
			}else if (events[x+1]['priority'] == 4 ){
				events[x+1]['color'] = '#FFF700';
				events[x+1]['colorText'] = '#000000';
			}else if (events[x+1]['priority'] == 5 ){
				events[x+1]['color'] = '#0C850A';
				events[x+1]['colorText'] = '#000000';
			}else if (events[x+1]['priority'] == 6 ){
				events[x+1]['color'] = '#00FF00';
				events[x+1]['colorText'] = '#000000';
			}
			
			//Color Conflicting Events Red
			if (events[x]['to'] > events[x+1]['from'] && events[x]['facility_id'].localeCompare(events[x+1]['facility_id']) == 0){
				
				events[x]['color'] = '#FF0000';
				events[x+1]['color'] = '#FF0000';
			}
				
			
			
			if (x > 0){
				if (events[x-1]['to'] > events[x]['from'] && events[x]['facility_id'].localeCompare(events[x-1]['facility_id']) == 0){
    ...
    function buildFullDayView() {
        ...
        for ( var hour = 0; hour < 24; hour++ ) {
            ...
            var newHour1 = createElement( "div", "text" );
            newHour1.innerText = padNumber( hour ) + ":00";
			newHour1.style.fontSize = "x-small";
			newHour1.style.fontWeight = "bold";
			newHour1.style.color = "black";
			newHour1.style.backgroundColor = "white";
			newHour1.setAttribute("align", "left");
            ...
            for ( var eventIndex1 = 0; eventIndex1 < eventsLength; eventIndex1++ ) {
                ...
                for ( var eventIndex2 = 0; eventIndex2 < eventsLength; eventIndex2++ ) {
                    if ( eventIndex2 !== eventIndex1 ) {
                        ...
                        if ( overlaps ) {
							counter += 1;
							var margin = counter * 155;
							
							event2.style.marginLeft = String(margin)+"px";
    ...
    function buildEventDropDownMenu() {
        ...
        if ( _options.manualEditingEnabled ) {
			...
			//Removed Duplicate From Drop Down
            ...
            buildMenuItemWithIcon( _element_DropDownMenu_Event, "ib-close-icon", _options.removeEventText, function() {
                ...
                var onYesEvent = function() {
					...
					var contents = "id=";
					contents += _element_DropDownMenu_Event_EventDetails.id;
					http.open("POST", "removeEvent.php", true);
					...
					if (this.readyState == 4 && this.status == 200) {
						...
						var current = String(Date.parse(_element_DropDownMenu_Event_EventDetails.from));
						var replace1 = (window.location.href.split('?'))[0];
						var replace2 = (window.location.href.split('?'))[1];
						var paramArray = replace2.split('&');
						var thirdParam = paramArray[2].split('=');
						var replacement = replace1+'?'+paramArray[0]+'&'+paramArray[1]+'&'+thirdParam[0]+'='+current;						
    ...
    function buildEventEditingDialog() {
		...
		/*var energySplitContainer = createElement( "div", "split" );
		contents.appendChild( energySplitContainer );
		createTextHeaderElement( energySplitContainer, "Energy Level:" );
		_element_EventEditorDialog_Energy = createElement( "input", null, "text" );
		energySplitContainer.appendChild(_element_EventEditorDialog_Energy );*/
		
		var ionSplitContainer = createElement( "div", "split" );
		contents.appendChild( ionSplitContainer );		
		createTextHeaderElement( ionSplitContainer, "ION:" );
		_element_EventEditorDialog_Ion = createElement( "input", null, "text" );
		_element_EventEditorDialog_Ion.readOnly = true;
		ionSplitContainer.appendChild( _element_EventEditorDialog_Ion );		
		
		var prioritySplitContainer = createElement( "div", "split" );
		contents.appendChild( prioritySplitContainer );
		createTextHeaderElement( prioritySplitContainer, "Priority:" );
		_element_EventEditorDialog_Priority = createElement( "input", null, "text" );
		prioritySplitContainer.appendChild( _element_EventEditorDialog_Priority );
    ...
    function buildEventEditorEventTabContent() {
		...
		_element_EventEditorDialog_Title.readOnly = true;
		...
		//Edited to remove All-Day and Show Alerts Check Boxes
    ...
    function buildEventEditorRepeatsTabContent() {
		...
		var radioButtonsDummyContainer = createElement( "div", "radioButtonsContainer" );
        _element_EventEditorDialog_RepeatEvery_Never = buildRadioButton( radioButtonsRepeatsContainer, "Delete Subevents", "RepeatType", repeatEveryEvent );
        
		//send other buttons to dummy container
		_element_EventEditorDialog_RepeatEvery_EveryDay = buildRadioButton( radioButtonsDummyContainer, _options.repeatsEveryDayText, "RepeatType", repeatEveryEvent );
        _element_EventEditorDialog_RepeatEvery_EveryWeek = buildRadioButton( radioButtonsDummyContainer, _options.repeatsEveryWeekText, "RepeatType", repeatEveryEvent );
        _element_EventEditorDialog_RepeatEvery_Every2Weeks = buildRadioButton( radioButtonsDummyContainer, _options.repeatsEvery2WeeksText, "RepeatType", repeatEveryEvent );
        _element_EventEditorDialog_RepeatEvery_EveryMonth = buildRadioButton( radioButtonsDummyContainer, _options.repeatsEveryMonthText, "RepeatType", repeatEveryEvent );
        _element_EventEditorDialog_RepeatEvery_EveryYear = buildRadioButton( radioButtonsDummyContainer, _options.repeatsEveryYearText, "RepeatType", repeatEveryEvent );
        
		_element_EventEditorDialog_RepeatEvery_Custom = buildRadioButton( radioButtonsRepeatsContainer, "Create Subevents", "RepeatType", repeatEveryEvent );
    ...
    function buildEventEditorExtraTabContent() {
		...
		_element_EventEditorDialog_Location.readOnly = true;
		...
		_element_EventEditorDialog_Group.readOnly = true;
		...
		_element_EventEditorDialog_Url.readOnly = true;
    ...
    function isAllDayChanged() {
        ...
        if ( toDate > fromDate || toDate < fromDate ) {
            disabled = true;
            _element_EventEditorDialog_RepeatEvery_Never.checked = false;
        } else {
            disabled = false;
        }
    ...
    function showEventEditingDialog( eventDetails, overrideTodayDate, overrideTimeValues ) {
        ...
        if ( isDefined( eventDetails ) ) {
			...
			if (eventDetails.approved == 0){
				_element_EventEditorDialog_OKButton.value = "Approve";
			}else{
			...
			//_element_EventEditorDialog_Energy.value = eventDetails.energy;			
			_element_EventEditorDialog_Ion.value = eventDetails.ion; 			
			_element_EventEditorDialog_Priority.value = eventDetails.priority;			
			_element_EventEditorDialog_RepeatEvery_Never.checked = false;
			
            //_element_EventEditorDialog_OKButton.value = _options.updateText;
            _element_EventEditorDialog_RemoveButton.style.display = "block";
            _element_EventEditorDialog_TitleBar.innerText = "Facility: "+eventDetails.location;
            ...
            _element_EventEditorDialog_RepeatEvery_Never.checked = false;
            _element_EventEditorDialog_RepeatEvery_EveryDay.checked = false;
            _element_EventEditorDialog_RepeatEvery_EveryWeek.checked = false;
            _element_EventEditorDialog_RepeatEvery_Every2Weeks.checked = false;
            _element_EventEditorDialog_RepeatEvery_EveryMonth.checked = false;
            _element_EventEditorDialog_RepeatEvery_EveryYear.checked = false;
            _element_EventEditorDialog_RepeatEvery_Custom.checked = false;
            _element_EventEditorDialog_RepeatEvery_Custom_Type_Daily.checked = false;
            _element_EventEditorDialog_RepeatEvery_Custom_Type_Weekly.checked = false;
            _element_EventEditorDialog_RepeatEvery_Custom_Type_Monthly.checked = false;
            _element_EventEditorDialog_RepeatEvery_Custom_Type_Yearly.checked = false;
            ...
            _element_EventEditorDialog_ShowAlerts.checked = false;
    ...
    function eventDialogEvent_OK() {
		...
		if (_element_EventEditorDialog_EventDetails.approved == 0){
			_element_EventEditorDialog_EventDetails.approved = 1;
			...
			if ( toDate < fromDate ) {
				...
				var isExistingEvent = isDefined( _element_EventEditorDialog_EventDetails.id ),
					newEvent = {
						...
						url: url,							
						//added here
						approved: _element_EventEditorDialog_EventDetails.approved,
						priority: _element_EventEditorDialog_Priority.value,
						ion: _element_EventEditorDialog_EventDetails.ion,
						energy: _element_EventEditorDialog_EventDetails.energy,
						request_id: _element_EventEditorDialog_EventDetails.request_id,
						facility_id: _element_EventEditorDialog_EventDetails.facility_id,
						total_hours: _element_EventEditorDialog_EventDetails.total_hours
					};
				...
				if ( isExistingEvent ) {
					...
					if (_element_EventEditorDialog_RepeatEvery_Never.checked){
						...
						var http = new XMLHttpRequest();
						var contents = "id=";
						contents += _element_EventEditorDialog_EventDetails.id;
						http.open("POST", "removeDupEvent.php", true);
						http.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');		

						http.onreadystatechange = function() {
						if (this.readyState == 4 && this.status == 200) {
							
							var newEvent = cloneEventDetails( _element_EventEditorDialog_EventDetails );
							newEvent.id = "1-1";
						...
						//Stop DOM from creating repeated events
						_element_EventEditorDialog_RepeatEvery_Custom.checked == false;				
						...
						if (this.readyState == 4 && this.status == 200) {		
							...
							//newEvent.from.setHours(newEvent.from.getHours() - (offset));							
							uploadDupEvent(newEvent);
					...
					_this.addEvent( newEvent, true, false );
    ...
    function eventDialogEvent_Remove() {
        ...
        var onYesEvent = function() {
			...
			if (this.readyState == 4 && this.status == 200) {
				...
				var current = String(Date.parse(_element_EventEditorDialog_EventDetails.from));
				var replace1 = (window.location.href.split('?'))[0];
				var replace2 = (window.location.href.split('?'))[1];
				var paramArray = replace2.split('&');
				var thirdParam = paramArray[2].split('=');
    ...
    function getExportFileAttributes( exportType ) {
		...
		// comment back in once individual event is updated
    ...
    /*
     * ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
     * Exporting To JSON : Completly Overwritten for Database
     * ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
     */
   ...
   /* function getJsonContents( orderedEvents ) {
        ...
        for ( var orderedEventIndex = 0; orderedEventIndex < orderedEventLength; orderedEventIndex++ ) {
            ...
            for ( var propertyNameIndex = 0; propertyNameIndex < propertyNamesLength; propertyNameIndex++ ) {
                ...
                if ( orderedEvent.hasOwnProperty( propertyName ) && orderedEvent[ propertyName ] !== null ) {
                    contents += ( "\"" + propertyName + "\":" + getPropertyValue( propertyName, orderedEvent[ propertyName ], true )+",");
                }
            }
		...
		var LINE_EXPRESSION = /\r\n|\n\r|\n|\r/g;
		contents = contents.replace(LINE_EXPRESSION, '');
		contents = contents.slice( 0, -1 );
        contents += "]";
		...
		//database export
		var http = new XMLHttpRequest();	
		http.open("POST", "commitEvents.php", true);
		http.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');		
	...
	*/
     ...
     */
	 ...
	 function uploadEvent ( event ){
		...
		var date1 = event.from;
		var date2 = event.to;
		
		var offset = (new Date()).getTimezoneOffset()/60;							
		event.from.setHours(event.from.getHours() - (offset));
		event.to.setHours(event.to.getHours() - (offset));
		event.from = event.from.toISOString().split("Z")[0];
		event.to = event.to.toISOString().split("Z")[0];
		...
		if (this.readyState == 4 && this.status == 200) {
			...
			if (event.id === "1-1"){
				var current = String(Date.parse(event.from));
				var replace1 = (window.location.href.split('?'))[0];
				var replace2 = (window.location.href.split('?'))[1];
				var paramArray = replace2.split('&');
				var thirdParam = paramArray[2].split('=');
	...
	function uploadDupEvent ( event ){
		...
		//database export
		var http = new XMLHttpRequest();	
		http.open("POST", "commitDupEvent.php", true);
		http.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');		
    ...
    this.addEvent = function( event, updateEvents, triggerEvent, setLastUpdated ) {
		...
		//set All Day to false for all events. Reduction requried since mySQL will not return boolean as boolean from JSON
        event.isAllDay = false;
		event.repeatEvery = parseInt(event.repeatEvery);
		event.repeatEveryCustomType = parseInt(event.repeatEveryCustomType);
		event.repeatEveryCustomValue = parseInt(event.repeatEveryCustomValue);
        ...
        if ( event.from <= event.to ) {
            ...
            if ( !_events[ storageDate ].hasOwnProperty( storageGuid ) ) {
				...
				if ( !isDefined( event.approved ) ) {
                    event.approved = 0;
                }
				
				if ( !isDefined( event.priority ) ) {
                    event.priority = 0;
                }
    ...
    this.updateEvent = function( id, event, updateEvents, triggerEvent ) {
        ...
        if ( updated ) {
            ...
            updated = this.addEvent( event, true, false );
			//Added to update database
			//From the Update Button
			//this.exportAllEvents('json');
    ...
    function setTranslationStringOptions() {
        ...
        if ( !isDefinedString( _options.titleText ) ) {
            _options.titleText = "Organization:";
        }
        ...
        if ( !isDefinedString( _options.groupText ) ) {
            _options.groupText = "Test Requester:";
        }
        ...
        if ( !isDefinedString( _options.optionalText ) ) {
            _options.optionalText = "Information";
        }

        if ( !isDefinedString( _options.urlText ) ) {
            _options.urlText = "Link to Request Editor:";
        }

        if ( !isDefinedString( _options.openUrlText ) ) {
            _options.openUrlText = "Edit Request";
        }
